...
CASES = [
    ...
    {"id": "orphan_open_paren_kept_content", "raw": "Duration 7m (RX7) and (c) 2026 build (RED)",
     "require": ["(RX7)", "(c)", "(RED)"], "forbid": [], "forbid_re": [],
     "note": "R5 SAFETY: content-bearing parens (a non-separator follows '(') survive — only '(' immediately before ',;:' is orphan residue (token kept unit-free so it tests both stages)"},
    {"id": "approx_time_seconds", "stage": "pipeline", "raw": "the run took ~1.1s warm", "require": ["about 1.1 seconds"],
     "forbid": ["~", "1.1s"], "forbid_re": [],
     "note": "NUMERIC (pipeline): '~' before a number reads 'about'; '1.1s' expands to 'seconds' (engine voiced bare '~'/'s' as gibberish 'tilda one ones')"},
    {"id": "millis_unit", "stage": "pipeline", "raw": "judge latency was 150ms steady", "require": ["150 milliseconds"],
     "forbid": ["150ms"], "forbid_re": [],
     "note": "NUMERIC (pipeline): 'ms' abutting a number expands to 'milliseconds'"},
    {"id": "percent_unit", "stage": "pipeline", "raw": "markup dropped to 24.0% after the fix", "require": ["24.0 percent"],
     "forbid": ["24.0%"], "forbid_re": [],
     "note": "NUMERIC (pipeline): trailing '%' on a number reads 'percent'"},
    {"id": "double_percent", "stage": "pipeline", "raw": "progress reached 100%% today", "require": ["100 percent today"],
     "forbid": ["percent%", "100%%"], "forbid_re": [],
     "note": "NUMERIC (pipeline): a printf '%%' run after a number collapses to one 'percent', no orphan '%' (adversarial-verify)"},
    {"id": "singular_second", "stage": "pipeline", "raw": "it slept 1s then retried", "require": ["1 second"],
     "forbid": [], "forbid_re": [r"\b1 seconds\b"],
     "note": "NUMERIC (pipeline): exactly '1s' is singular 'one second', not 'seconds'"},
    {"id": "prose_unit_safety", "raw": "discuss the seconds and minutes, plus the surplus",
     "require": ["the seconds and minutes", "plus the surplus"], "forbid": [], "forbid_re": [],
     "note": "NUMERIC SAFETY: units expand only when abutting a digit — prose 'seconds'/'plus'/'surplus' (no digit) untouched"},
    {"id": "decade_not_seconds", "raw": "The 2020s were wild and the 90s too",
     "require": ["2020s", "90s"], "forbid": ["2020 seconds", "90 seconds"], "forbid_re": [],
     "note": "NUMERIC SAFETY: 4-digit year-decade '2020s' and era-prefixed 'the 90s' are NOT seconds (adversarial-verify defect)"},
    {"id": "plural_not_seconds", "raw": "saw 100s of errors and 1000s of files",
     "require": ["100s of errors", "1000s of files"], "forbid": ["100 seconds", "1000 seconds"], "forbid_re": [],
     "note": "NUMERIC SAFETY: informal plural '<n>s of' is not a duration (adversarial-verify defect)"},
    {"id": "percent_letter_glue", "raw": "pegged at 100%CPU and 50%off",
     "require": ["100%CPU"], "forbid": ["percentCPU", "percentoff"], "forbid_re": [],
     "note": "NUMERIC SAFETY: '%' abutting a letter is left alone — never glue an unspeakable 'percentCPU' (adversarial-verify defect)"},
    {"id": "identifier_not_unit", "raw": "timeout_30s exceeded and timeout=5s set",
     "require": ["timeout_30s", "timeout=5s"], "forbid": ["timeout_30 seconds", "timeout=5 seconds"], "forbid_re": [],
     "note": "NUMERIC SAFETY: identifiers / key=value (number preceded by word-char or '=') are not unit tokens (adversarial-verify defect)"},
    {"id": "compound_duration_safe", "raw": "ran 3m24s total in 1m13s each",
     "require": ["3m24s", "1m13s"], "forbid": ["24 seconds", "13 seconds"], "forbid_re": [],
     "note": "NUMERIC SAFETY: interior digit of a compound token (3m24s) is not re-matched into a unit (adversarial-verify defect)"},
